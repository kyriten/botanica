        ...
        <div class="row">
            <div class="col-lg-4 d-flex justify-content-start align-items-center mb-3">
                <a class="btn btn-primary fw-bold text-light"
                    href="{{ route('city.create') }}">{{ __('Kota/Kabupaten Baru') }}</a>
            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        @forelse ($cities as $item)
                                            @php
                                                $iterationNumber =
                                                    ($cities->currentPage() - 1) * $cities->perPage() +
                                                    $loop->index +
                                                    1;
                                            @endphp
                                            <tr>
                            ...
                            @if ($cities->hasPages())
                                <nav aria-label="Pagination">
                                    <ul class="pagination justify-content-center pagination-circle">
                                        ...
                                        @else
                                            <li class="page-item">
                                                <a class="page-link" href="{{ $cities->previousPageUrl() }}">Sebelumnya</a>
                                            </li>
                                        @endif
                                        ...
                                        @if ($start > 1)
                                            <li class="page-item"><a class="page-link" href="{{ $cities->url(1) }}">1</a>
                                            </li>
                                            @if ($start > 2)
                                        ...
                                        @if ($end < $lastPage)
                                            ...
                                            @endif
                                            <li class="page-item"><a class="page-link"
                                                    href="{{ $cities->url($lastPage) }}">{{ $lastPage }}</a></li>
                                        @endif

                                        {{-- Next Page Link --}}
                                        @if ($cities->hasMorePages())
                                            <li class="page-item">
                                                <a class="page-link" href="{{ $cities->nextPageUrl() }}">Selanjutnya</a>
                                            </li>
                                        @else
