...
<main id="main" class="main">
    <div class="pagetitle d-none d-md-block">
        <div class="row">
            <h2 class="text-dark fw-bold text-wrap mb-4">Data Kota/Kabupaten di Indonesia</h2>
        </div>
    </div><!-- End Page Title -->
    ...
    <div class="row align-items-center gy-2 mb-3">
                ...
                <div class="col-12 col-md-8">
                    ...
                    <div class="d-none d-md-flex flex-wrap gap-2">
                        ...
                        <a class="btn btn-sm btn-botanica fw-bold text-light d-flex align-items-center gap-2"
                            href="{{ route('city.create') }}">
                            <i class="fas fa-plus"></i>
                            <span class="d-none d-md-inline">Kota/Kabupaten Baru</span>
                        </a>
        ...
        <div class="col-9 col-md-8 col-lg-9 d-flex justify-content-end">
            <form action="{{ route('city.index') }}" method="GET" class="search-form">
                <div class="input-group rounded toggle-search w-100" id="searchContainer">
                    <input type="text" name="search" class="form-control" id="searchInput"
                        placeholder="Cari kota/kabupaten" value="{{ request('search') }}">
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    @forelse ($cities as $item)
                                        @php
                                            $iterationNumber = ($cities->currentPage() - 1) * $cities->perPage() + $loop->index + 1;
                                        @endphp
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex gap-3">
                                                    ...
                                                    <form action="{{ route('city.destroy', $item->id) }}" method="post" class="d-inline">
                                                        @method('delete')
                                                        @csrf
                        ...
                        @if ($cities->hasPages())
                            <nav aria-label="Pagination">
                                <ul class="pagination justify-content-center pagination-circle mt-2">
                                    ...
                                    @else
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $cities->previousPageUrl() }}"><i class="bi bi-chevron-double-left"></i></a>
                                        </li>
                                    @endif
                                    ...
                                    @if ($start > 1)
                                        <li class="page-item"><a class="page-link" href="{{ $cities->url(1) }}">1</a></li>
                                        @if ($start > 2)
                                    ...
                                    @if ($end < $lastPage)
                                        @if ($end < $lastPage - 1)
                                            <li class="page-item disabled"><span class="page-link">...</span></li>
                                        @endif
                                        <li class="page-item"><a class="page-link" href="{{ $cities->url($lastPage) }}">{{ $lastPage }}</a></li>
                                    @endif

                                    @if ($cities->hasMorePages())
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $cities->nextPageUrl() }}"><i class="bi bi-chevron-double-right"></i></a>
                                        </li>
                                    @else
