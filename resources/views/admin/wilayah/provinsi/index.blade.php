                ...
                <div class="col-12 col-md-8">
                    ...
                    <div class="collapse d-md-none mt-2" id="actionButtonsCollapse">
                        <div class="d-flex flex-column gap-2">
                            ...
                            <a class="btn btn-sm btn-botanica fw-bold text-light d-flex align-items-center justify-content-center gap-2 w-100"
                                href="{{ route('province.create') }}">
                                <i class="fas fa-plus"></i>
                            </a>

                            {{-- Tombol Select Wilayah --}}
                            <div class="dropdown w-100">
                                <button class="btn btn-secondary btn-sm d-flex align-items-center justify-content-center gap-2 w-100 no-caret"
                                ...
                                <ul class="dropdown-menu w-100" aria-labelledby="selectWilayahDropdownMobile">
                                    <li><a class="dropdown-item" href="{{ route('city.index') }}">Kota/Kabupaten</a></li>
                                    <li><a class="dropdown-item" href="{{ route('district.index') }}">Kecamatan</a></li>
                                    <li><a class="dropdown-item" href="{{ route('village.index') }}">Kelurahan</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-none d-md-flex flex-wrap gap-2">
                        ...
                        <a class="btn btn-sm btn-botanica fw-bold text-light d-flex align-items-center gap-2"
                            href="{{ route('province.create') }}">
                            <i class="fas fa-plus"></i>
                            <span class="d-none d-md-inline">Provinsi Baru</span>
                        </a>
                        ...
                        <div class="dropdown">
                            <button class="btn btn-secondary dropdown-toggle btn-sm d-flex align-items-center gap-2"
                                ...
                                aria-expanded="false">
                                <i class="bi bi-arrow-left-right"></i>
                                <span class="d-none d-md-inline">Pilih Wilayah</span>
                            </button>
            ...
            <form action="{{ route('province.index') }}" method="GET" class="search-form">
                <div class="input-group rounded toggle-search w-100" id="searchContainer">
                    <input type="text" name="search" class="form-control" id="searchInput"
                        placeholder="Cari provinsi" value="{{ request('search') }}">
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    @forelse ($provinces as $item)
                                        @php
                                            $iterationNumber = ($provinces->currentPage() - 1) * $provinces->perPage() + $loop->index + 1;
                                        @endphp
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex gap-3">
                                                    {{-- Contoh tombol edit (bisa diaktifkan jika ada route edit) --}}
                                                    {{-- <a class="btn btn-warning" href="{{ route('province.edit', $item->id) }}">
                                                        <i class="bi bi-pencil"></i>
                                                    </a> --}}
                                                    <form action="{{ route('province.destroy', $item->id) }}" method="post" class="d-inline">
                                                        @method('delete')
                                                        @csrf
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="8" class="text-center">Tidak ada data provinsi.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                        ...
                        @if ($provinces->hasPages())
                            <nav aria-label="Pagination">
                                <ul class="pagination justify-content-center pagination-circle mt-2">
                                    ...
                                    @else
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $provinces->previousPageUrl() }}"><i class="bi bi-chevron-double-left"></i></a>
                                        </li>
                                    @endif
                                    ...
                                    @if ($start > 1)
                                        <li class="page-item"><a class="page-link" href="{{ $provinces->url(1) }}">1</a></li>
                                        @if ($start > 2)
                                    ...
                                    @for ($i = $start; $i <= $end; $i++)
                                        ...
                                        @else
                                            <li class="page-item"><a class="page-link" href="{{ $provinces->url($i) }}">{{ $i }}</a></li>
                                        @endif
                                    @endfor
                                    ...
                                    @if ($end < $lastPage)
                                        @if ($end < $lastPage - 1)
                                            <li class="page-item disabled"><span class="page-link">...</span></li>
                                        @endif
                                        <li class="page-item"><a class="page-link" href="{{ $provinces->url($lastPage) }}">{{ $lastPage }}</a></li>
                                    @endif

                                    @if ($provinces->hasMorePages())
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $provinces->nextPageUrl() }}"><i class="bi bi-chevron-double-right"></i></a>
                                        </li>
                                    @else
